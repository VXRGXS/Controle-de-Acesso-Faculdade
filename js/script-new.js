class AccessControl {
    constructor() {
        ...
        this.loadStatsFromLocalStorage(); // Carrega as estatísticas do localStorage, se existirem
        this.fetchDailyStats(); // Busca as estatísticas do dia ao iniciar
        this.initializeEventListeners(); // Inicializa os ouvintes de eventos
        this.updateStats(); // Atualiza as estatísticas na interface
    }

    // Carrega as estatísticas do localStorage, se existirem
    loadStatsFromLocalStorage() {
        const storedEntries = localStorage.getItem('totalEntries');
        const storedExits = localStorage.getItem('totalExits');
        const storedPresenteCount = localStorage.getItem('presentes');

        if (storedEntries) {
            this.totalEntries = parseInt(storedEntries);
        }
        if (storedExits) {
            this.totalExits = parseInt(storedExits);
        }
        if (storedPresenteCount) {
            this.presentStudents = new Set(JSON.parse(storedPresenteCount));
        }
    }
    ...
    fetchDailyStats() {
        fetch('listar_acessos.php')
            ...
            .then(data => {
                ...
                this.presentStudents = new Set(data.presentes);

                // Armazena as estatísticas no localStorage
                localStorage.setItem('totalEntries', this.totalEntries);
                localStorage.setItem('totalExits', this.totalExits);
                localStorage.setItem('presentes', JSON.stringify([...this.presentStudents]));
    ...
    processServerResponse(data, studentId, accessType) {
        if (data.status === 'sucesso') {
            ...
            // Atualiza as estatísticas no localStorage
            localStorage.setItem('totalEntries', this.totalEntries);
            localStorage.setItem('totalExits', this.totalExits);
            localStorage.setItem('presentes', JSON.stringify([...this.presentStudents]));
        } else {
